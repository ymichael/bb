...
describe("internal event side effects", () => {
  ...
  it("does not reactivate an interrupted thread from a stale turn/started event", async () => {
    const harness = await createTestAppHarness();
    try {
      const { host, session } = seedHostSession(harness.deps, {
        id: "host-event-interrupted-start",
      });
      const { project } = seedProjectWithSource(harness.deps, { hostId: host.id });
      const environment = seedEnvironment(harness.deps, {
        hostId: host.id,
      ...
      seedThreadRuntimeState(harness.deps, {
        environmentId: environment.id,
        providerThreadId: "provider-interrupted-start",
        sequenceStart: 1,
        threadId: thread.id,
      });
      seedEvent(harness.deps, {
  ...
  it("allows turn/started after a newer client request follows an interruption", async () => {
    const harness = await createTestAppHarness();
    try {
      const { host, session } = seedHostSession(harness.deps, {
        id: "host-event-interrupted-new-request",
      });
      const { project } = seedProjectWithSource(harness.deps, { hostId: host.id });
      const environment = seedEnvironment(harness.deps, {
        hostId: host.id,
      ...
      seedEvent(harness.deps, {
        threadId: thread.id,
        environmentId: environment.id,
        providerThreadId: "provider-interrupted-new-request",
        sequence: 1,
        type: "thread/identity",
        data: {},
      });
      ...
      seedEvent(harness.deps, {
        ...
        data: turnRequestEventDataSchema.parse({
          direction: "outbound",
          input: [
            {
              type: "text",
              text: "Continue after interruption",
            },
          ],
          execution: {
            model: "gpt-5",
          ...
          events: [
            {
              ...
              event: {
                ...
                threadId: thread.id,
                providerThreadId: "provider-interrupted-new-request",
                turnId: "turn-interrupted-new-request",
              },
            },
          ],
        }),
