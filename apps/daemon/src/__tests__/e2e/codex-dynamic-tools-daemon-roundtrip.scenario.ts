...
export async function runCodexDynamicToolsDaemonRoundtripScenario(): Promise<void> {
  ...
  try {
        ...
        input: [
          {
            type: "text",
            text:
              'Use echo_test_tool with message "from-daemon-codex". After the tool returns, reply with exactly the returned text and nothing else.',
          },
        ],
      }),
    });
    ...
    expect(output?.toLowerCase()).toContain("from-daemon-codex");
