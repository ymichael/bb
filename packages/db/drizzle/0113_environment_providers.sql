...
UPDATE `hosts`
SET
  `machine_provider_id` = 'modal-sandbox',
  `machine_provider_selection` = json_object('inputs', json('null')),
  `phase` = CASE
    WHEN EXISTS (
      SELECT 1 FROM `plugin_kv`
      WHERE `plugin_kv`.`plugin_id` = 'environment-modal-sandbox'
        AND json_valid(`plugin_kv`.`value`)
        AND json_extract(`plugin_kv`.`value`, '$.hostId') = `hosts`.`id`
        AND json_extract(`plugin_kv`.`value`, '$.phase') = 'hibernated'
    ) THEN 'suspended'
    ELSE 'active'
  END,
  `suspended_at` = CASE
  ...
  `resource` = (
    SELECT json_object(
      'version', 2,
      'key', json_extract(`plugin_kv`.`value`, '$.pathKey'),
      'sandboxId', json_extract(`plugin_kv`.`value`, '$.sandboxId'),
      'snapshotImageId', json_extract(`plugin_kv`.`value`, '$.snapshotImageId'),
      'projectId', json_extract(`plugin_kv`.`value`, '$.projectId'),
      'sourceId', json_extract(`plugin_kv`.`value`, '$.sourceId')
    )
    FROM `plugin_kv`
    WHERE `plugin_kv`.`plugin_id` = 'environment-modal-sandbox'
      AND json_valid(`plugin_kv`.`value`)
      AND json_extract(`plugin_kv`.`value`, '$.hostId') = `hosts`.`id`
      AND json_extract(`plugin_kv`.`value`, '$.pathKey') IS NOT NULL
    ORDER BY `plugin_kv`.`updated_at` DESC
    LIMIT 1
  )
...
SET
  ...
  `resource` = NULL
WHERE `host_id` IN (
  SELECT `id` FROM `hosts` WHERE `machine_provider_id` = 'modal-sandbox'
);
--> statement-breakpoint
UPDATE `environments`
SET `is_worktree` = 1
