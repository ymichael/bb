...
describe("environment-daemon repositories", () => {
  ...
  it("refreshes active session metadata without replacing the session row", () => {
    ...
    sessions.create({
      id: "sess-refresh",
      environmentId,
      environmentDaemonId: "agent-refresh",
      environmentDaemonInstanceId: "instance-refresh",
      protocolVersion: 1,
      providerMetadata: [{ providerId: "codex", adapterVersion: "0.0.1" }],
  ...
  it("expires and closes sessions idempotently", () => {
    ...
    const created = sessions.create({
      id: "sess-expire",
      environmentId,
      environmentDaemonId: "agent-expire",
      environmentDaemonInstanceId: "instance-expire",
      protocolVersion: 1,
  ...
  it("throws for invalid persisted session transport/status values", () => {
    ...
    sessions.create({
      id: "sess-invalid",
      environmentId,
      environmentDaemonId: "agent-invalid",
      environmentDaemonInstanceId: "instance-invalid",
      protocolVersion: 1,
      selectedCapabilities: { commands: [], features: [] },
      leaseExpiresAt: 5_000,
      now: 1_000,
  ...
  it("enqueues commands with monotonic per-thread cursors and tracks deliverable state", () => {
    ...
    sessions.create({
      id: "sess-cmd",
      environmentId,
      environmentDaemonId: "agent-cmd",
      environmentDaemonInstanceId: "instance-cmd",
      protocolVersion: 1,
      leaseExpiresAt: 5_000,
      now: 1_000,
    });
  ...
  it("keeps received commands bound to their original session", () => {
    ...
    sessions.create({
      ...
      environmentId,
      environmentDaemonId: "agent-cmd",
      environmentDaemonInstanceId: "instance-old",
      protocolVersion: 1,
      leaseExpiresAt: 5_000,
      now: 1_000,
    });
    ...
    sessions.create({
      ...
      environmentId,
      environmentDaemonId: "agent-cmd",
      environmentDaemonInstanceId: "instance-new",
      protocolVersion: 1,
