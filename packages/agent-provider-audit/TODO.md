...
- Fixture replay is now reproducible offline. The package can regenerate thread events, semantic timeline rows, CLI text, and audit summaries without live provider usage.
...
- Provider-agnostic thread events and projected timeline rows now preserve optional `parentToolCallId` linkage where the provider emits it.
- The semantic timeline has first-class delegation work rows instead of forcing subagent activity through generic tool-call rendering.
- Task/todo/plan updates are intentionally out of the timeline until they have a separate surface.
- Low-value `TodoRead` churn is now suppressed instead of leaking into the rendered timeline.
- The CLI/audit timeline renderer now understands delegation rows, and recursive nested activity rendering is supported when parent-child linkage is present.
- `@bb/agent-provider-audit` now generates fixture data that can feed a future React visual audit after the `ui-core` design-system work lands.
...
- The React timeline renderer is intentionally deferred until `packages/ui-core` has a stable design-system foundation.
